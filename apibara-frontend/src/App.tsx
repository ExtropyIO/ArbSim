...
const App = () => {
  ...
  useEffect(() => {
    const login = async () => {
      const loggedInUser = await app.logIn(Realm.Credentials.anonymous());
      ...
      const databaseName = process.env.REACT_APP_MONGO_DATABASE_NAME;
      if (!databaseName) {
        console.error('The REACT_APP_MONGO_DATABASE_NAME environment variable is not set.');
        return;
      }

      const mongodb = app.currentUser.mongoClient("mongodb-atlas");
      const collection = mongodb.db(databaseName).collection("swaps");
